...
fig_num: int = 4  # 3 or 4 showing 1 / many runs

if fig_num == 3:
    subset_seeds: list[int] = [1]
    init_step_sizes: List[float] = [1.5, 1.0, 0.3]
elif fig_num == 4:
    subset_seeds: list[int] = [1, 2, 3, 4, 5]
    init_step_sizes: List[float] = [1.0]
else:
    raise ValueError("fig_num must be 3 or 4")
...
for method in methods:
            ...
            for i_n, num_subsets in enumerate(num_subsets_list):
                for i_s, init_step_size in enumerate(init_step_sizes):
                    for i_ss, subset_seed in enumerate(subset_seeds):
                        ...
                        if i_ss == 0:
                            label = (
                                f"$n_s$={num_subsets} $\\tau_0$={init_step_size:.1f}"
                            )
                        else:
                            label = None
    ...
    if len(subset_seeds) == 1:
        oname = f"fig3_{method}_{len(subset_seeds)}runs_tof_{tof}.pdf"
    else:
        oname = f"fig4_{method}_{len(subset_seeds)}runs_tof_{tof}.pdf"
